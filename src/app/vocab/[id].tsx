...
import { FlatList, StyleSheet, View, Text, TouchableOpacity, Modal, TextInput, Alert, ScrollView, PanResponder, Keyboard, TouchableWithoutFeedback, Animated } from 'react-native';
...
export default function VocabScreen() {
  ...
  const handleSaveWord = async () => {
    ...
    if (editingWordId) {
      ...
      if (currentWordInState && currentWordInState.word !== trimmedWord) {
        ...
        if (isDuplicate) {
          Alert.alert('Duplicate Found', `The word "${trimmedWord}" already exists in this set.`);
          return;
        }
      }
      ...
      if (res.success) {
        setVocabData((prev) => 
          prev.map((item) => item.id === editingWordId ? {
            ...item,
            word: trimmedWord,
            reading: reading.trim(),
            meaning: meaning.trim(),
      ...
      const updatedManifest = manifest.map((m) => 
        m.id === setId ? { ...m, totalWords: newData.length, updatedAt: new Date().toISOString() } : m
      );
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      Alert.alert('Export Error', 'An error occurred while trying to share the file.');
    }
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#000' : '#f5f5f5' }]} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRightActions}>
          {vocabData.length > 0 && (
            <TouchableOpacity style={styles.headerBtn} onPress={() => router.push(`/vocab/practice?setId=${setId}` as any)}>
              <Ionicons name="play-circle-outline" size={30} color={isDark ? '#34C759' : '#34C759'} />
            </TouchableOpacity>
          )}
          <TouchableOpacity style={styles.headerBtn} onPress={handleExport}>
            <Ionicons name="share-outline" size={28} color={isDark ? '#fff' : '#000'} />
          </TouchableOpacity>
          <TouchableOpacity style={styles.headerBtn} onPress={handleOpenCategoryEdit}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <ThemedText style={styles.emptyText}>No words saved yet.</ThemedText>
            <ThemedText style={styles.emptyTextSub}>Tap the + button to add vocabulary!</ThemedText>
          </View>
        }
      />
        ...
        <TouchableWithoutFeedback onPress={() => {
          ...
          <View style={styles.modalBackground}>
            <Animated.View style={[
                ...
                { transform: [{ translateY: wordModalY.interpolate({ inputRange: [0, 800], outputRange: [0, 800], extrapolate: 'clamp' }) }] }
              ]} 
            ...
            {isViewMode ? (
              <View style={styles.viewModeContainer}>
                ...
                {!!wordType && (
                  <View style={[styles.viewTypeBadge, { backgroundColor: isDark ? '#2C2C2E' : '#E5E5EA' }]}>
                    <Text style={[styles.viewTypeBadgeText, { color: isDark ? '#E5E5EA' : '#48484A' }]}>{wordType.split(' ')[0]}</Text>
                  </View>
                )}
                
                {!!reading && (
                  <ThemedText style={styles.viewReading}>{reading}</ThemedText>
                )}
                
                <ThemedText style={styles.viewMeaning}>{meaning}</ThemedText>
                
                {!!source && (
                  <View style={styles.viewSourceBox}>
                    <ThemedText style={styles.viewSourceText}>Note: {source}</ThemedText>
                  </View>
                )}
              </View>
            ) : (
              <View>
                ...
                <ThemedText type="default" style={styles.sectionLabel}>Word Type</ThemedText>
                    ...
                    {PART_OF_SPEECH_OPTIONS.map((opt) => {
                      ...
                      return (
                        ...
                        >
                          <Text style={[
                            styles.chipText,
                            isSelected ? styles.chipTextSelected : { color: isDark ? '#ccc' : '#666' }
                          ]}>{opt}</Text>
                        </TouchableOpacity>
                      );
                    })}
            ...
            <View style={[styles.modalActions, (editingWordId && !isViewMode) ? { justifyContent: 'space-between' } : { justifyContent: 'flex-end'} ]}>
              {(editingWordId && !isViewMode) ? (
                <TouchableOpacity style={styles.deleteModalBtn} onPress={() => handleDeleteWord(editingWordId)}>
              ...
              {isViewMode ? (
                <View style={{flexDirection: 'row', gap: 12}}>
                  ...
                  <TouchableOpacity style={[styles.createBtn, { backgroundColor: '#0274DF' }]} onPress={() => setIsViewMode(false)}>
                ...
                <View style={{flexDirection: 'row', gap: 12}}>
                  ...
                  <TouchableOpacity style={styles.createBtn} onPress={handleSaveWord}>
      ...
      {/* Edit Category Modal */}
      <Modal visible={isCatModalVisible} transparent animationType="fade" onRequestClose={() => setCatModalVisible(false)}>
        <TouchableWithoutFeedback onPress={() => {
          Keyboard.dismiss();
          setCatModalVisible(false);
        }}>
          <View style={[styles.modalBackground, { justifyContent: 'center' }]}>
            <Animated.View style={[
                ...
                { backgroundColor: isDark ? '#1C1C1E' : '#fff' },
                { transform: [{ translateY: catModalY.interpolate({ inputRange: [0, 800], outputRange: [0, 800], extrapolate: 'clamp' }) }] }
              ]} 
            ...
            <View style={{marginBottom: 24}}>
              <TextInput
                style={[styles.input, { color: isDark ? '#fff' : '#000', borderColor: isDark ? '#333' : '#ccc', marginBottom: 0 }]}
            ...
            <View style={[styles.modalActions, { justifyContent: 'space-between' }]}>
              <TouchableOpacity style={styles.deleteModalBtn} onPress={handleDeleteCategory}>
                <Ionicons name="trash" size={20} color="#FF3B30" />
              ...
              <View style={{flexDirection: 'row', gap: 12}}>
                ...
                <TouchableOpacity style={styles.createBtn} onPress={handleUpdateCategory}>
                  <Text style={styles.createBtnText}>Save</Text>
                </TouchableOpacity>
              </View>
            </View>
